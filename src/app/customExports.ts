...
export function eventDispatcher(   devObj:{event:string,element:HTMLElement | Window}   ){
...
(
    ...
    if(   devObj.intent ==='add' && devObj.hook !== 'prepare'   ){
        ...
        */
        ...
        let {position} = devObj        
        let {group,index,stackSpacing,zChild,symbols,positionFn} = position
        group.extras[index] ={ stackSpacing} 
        if(index === 0 ){
            group.extras[index].targetHeight = minMaxDelta({
                items:group.elements[index].map((y,j)=>{return y.element}), 
                min,max               
            })
            let y = group.extras[index]
            y.targetHeight.finalDelta =   y.targetHeight.delta + y.stackSpacing
        }
        else if( index !== 0){
            group.extras[index].targetHeight = objectCopy( group.extras[index-1].targetHeight)
            group.extras[index].prevHeight = minMaxDelta({
                items:group.symbols[index-1].map((y,j)=>{return zChild[y].element}),
                min,max     
            })
            let y = group.extras[index] 
            y.targetHeight.finalDelta += y.prevHeight.delta + y.stackSpacing
        }
        symbols.forEach(positionFn);
    }

    else if(devObj.intent === 'move'){
        let {move} = devObj
        let {group,zChild,moveFn,heightGetter,moveFnPost} = move

        // getting the height of the new  entries
        group.extras.forEach(heightGetter? heightGetter: (x,i) => {
            x.height =   minMaxDelta({
                items:group.symbols[i].map((y,j)=>{return zChild[y].element}),
                min,max
            })  
            x.height.finalDelta =   x.height.delta + x.stackSpacing                              
        }); 
        //
